    ...
    changes.forEach(([key, value, isEvent, keys]) => {
      ...
      const isLinear =
        (rootState == null
          ...
          ? void 0
          : _rootState$gl.outputEncoding) === THREE__namespace.LinearEncoding; // Deal with pointer events ...
      ...
      if (isEvent) {
        ...
        targetProp &&
        targetProp.set &&
        (targetProp.copy || targetProp instanceof THREE__namespace.Layers)
      ) {
        ...
        // https://github.com/pmndrs/react-three-fiber/issues/274
          ...
          // Layers have no copy function, we must therefore copy the mask property
            ...
            targetProp instanceof THREE__namespace.Layers &&
            value instanceof THREE__namespace.Layers
          )
        ...
        if (
          !isLinear &&
          currentInstance[key] instanceof THREE__namespace.Texture
        )
...
const createStore = (applyProps, invalidate, advance, props) => {
  ...
  if (shadows) {
    ...
    else gl.shadowMap.type = THREE__namespace.PCFSoftShadowMap;
  ...
  const rootState = create__default["default"]((set, get) => {
    ...
    const camera = isCamera
      ? cameraOptions
      : orthographic
      ? new THREE__namespace.OrthographicCamera(0, 0, 0, 0, 0.1, 1000)
      : new THREE__namespace.PerspectiveCamera(75, 0, 0.1, 1000);
    ...
    function getCurrentViewport(
      ...
      if (target instanceof THREE__namespace.Vector3) tempTarget.copy(target);
  ...
  rootState.subscribe(() => {
    ...
    if (size !== oldSize || viewport.dpr !== oldDpr) {
      ...
      if (
        !camera.manual &&
        !(internal.lastProps.camera instanceof THREE__namespace.Camera)
      ) {
        if (isOrthographicCamera(camera)) {
...
const Canvas = /*#__PURE__*/ React__namespace.forwardRef(function Canvas(
  ...
  const [containerRef, { width, height }] = useMeasure__default["default"]({
    scroll: true,
    debounce: {
      scroll: 50,
      resize: 0,
...
function useLoader(Proto, input, extensions, onProgress) {
  ...
  const results = useAsset.useAsset(
